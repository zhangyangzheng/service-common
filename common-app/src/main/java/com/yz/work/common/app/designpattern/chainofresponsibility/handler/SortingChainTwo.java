...
@Component
@Order(2)
public class SortingChainTwo implements IHandler<HandleParameter, HandleResult<Object>> {

    @Override
    public HandleResult<Object> handle(HandleParameter parameter) throws Exception {
        if (concreteHandler(parameter.getState(), ChainBitEnum.THE_SECOND_STEP.getBit())) {
            // do something
